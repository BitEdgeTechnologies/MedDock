...
import {
  interpretCtScan,
  type CtScanInterpreterOutput,
} from '@/ai/flows/ct-scan-analyzer-flow';
...
import { FileText, Lightbulb, Loader2, Upload, Scan } from 'lucide-react';
import { Skeleton } from '@/components/ui/skeleton';
import Image from 'next/image';

const formSchema = z.object({
  photoDataUri: z.string({ required_error: 'A CT Scan image is required.' }),
  clinicalContext: z.string().optional(),
});

export function CtScanAnalyzerForm() {
  const [result, setResult] = useState<CtScanInterpreterOutput | null>(null);
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    setResult(null);
    try {
      const interpretationResult = await interpretCtScan(values);
      setResult(interpretationResult);
    } catch (error) {
      console.error('Error interpreting CT Scan:', error);
      toast({
        title: 'An Error Occurred',
        description: 'Failed to interpret the CT Scan. Please try again later.',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
       ...
       <div className="grid md:grid-cols-2 gap-8">
        ...
        <div>
            <Form {...form}>
                ...
                <FormField
                    ...
                    render={({ field }) => (
                    <FormItem>
                        <FormLabel className="text-lg">CT Scan Image</FormLabel>
                        <FormControl>
                        <div className="space-y-4">
                            <div className="w-full aspect-square rounded-md bg-muted overflow-hidden flex items-center justify-center">
                            {photoPreview ? (
                                <Image src={photoPreview} alt="CT Scan preview" width={400} height={400} className="object-contain h-full w-full"/>
                            ) : (
                ...
                <FormField
                    ...
                    <FormItem>
                        ...
                        <FormControl>
                        <Textarea
                            placeholder="e.g., '55M with sudden onset severe headache.'"
        ...
        <div className="space-y-6">
            ...
            <>
                ...
                <CardHeader><CardTitle className="flex items-center gap-2 text-primary"><Scan/> Impression</CardTitle></CardHeader>
