...
import {
  interpretXray,
  type XrayInterpreterOutput,
} from '@/ai/flows/x-ray-interpreter-flow';
...
import { Bone, FileText, Lightbulb, Loader2, Upload } from 'lucide-react';
import { Skeleton } from '@/components/ui/skeleton';
import Image from 'next/image';

const formSchema = z.object({
  photoDataUri: z.string({ required_error: 'An X-ray image is required.' }),
  clinicalContext: z.string().optional(),
});

export function XrayInterpreterForm() {
  const [result, setResult] = useState<XrayInterpreterOutput | null>(null);
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    setResult(null);
    try {
      const interpretationResult = await interpretXray(values);
      setResult(interpretationResult);
    } catch (error) {
      console.error('Error interpreting X-ray:', error);
      toast({
        title: 'An Error Occurred',
        description: 'Failed to interpret the X-ray. Please try again later.',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
       ...
       <div className="grid md:grid-cols-2 gap-8">
        ...
        <div>
            <Form {...form}>
                ...
                <FormField
                    ...
                    render={({ field }) => (
                    <FormItem>
                        <FormLabel className="text-lg">X-Ray Image</FormLabel>
                        <FormControl>
                        <div className="space-y-4">
                            <div className="w-full aspect-square rounded-md bg-muted overflow-hidden flex items-center justify-center">
                            {photoPreview ? (
                                <Image src={photoPreview} alt="X-Ray preview" width={400} height={400} className="object-contain h-full w-full"/>
                            ) : (
                ...
                <FormField
                    ...
                    <FormItem>
                        ...
                        <FormControl>
                        <Textarea
                            placeholder="e.g., '70-year-old male with a 3-day history of productive cough and fever.'"
        ...
        <div className="space-y-6">
            ...
            <>
                ...
                <CardHeader><CardTitle className="flex items-center gap-2 text-primary"><Bone/> Impression</CardTitle></CardHeader>
